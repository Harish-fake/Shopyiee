...
#
#  This script
#    1. substitutes the real DB_PASSWORD into the application account that
#       schema.sql creates (the file ships with the placeholder
#       `change_me_app_password` so that it can also be applied by hand), and
#    2. loads the fabricated sample catalogue.
#
...
if [ ! -f "$SCHEMA_FILE" ] || [ ! -f "$SEED_FILE" ]; then
  echo "[init] ERROR: expected $SCHEMA_FILE and $SEED_FILE to be mounted read-only" >&2
  exit 1
fi

echo "[init] creating schema and application account (password from DB_PASSWORD)"
sed "s|${PLACEHOLDER}|$(escape_sed_replacement "$DB_PASSWORD")|g" "$SCHEMA_FILE" \
  | mysql "${MYSQL_ARGS[@]}"

# Belt and braces: make sure the account password matches even if the schema
